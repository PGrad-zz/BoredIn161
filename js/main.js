...
function main(texture) {
	...
	pair_sun_material = new THREE.MeshLambertMaterial({emissive: 0xff0000});
	sun.geometry = new THREE.SphereGeometry(12);
	let pair_sun_geometry = new THREE.SphereGeometry(20);
	...
	let pair_sun = new THREE.Mesh(pair_sun_geometry, pair_sun_material);
	...
	game.scene.add(pair_sun);
	game.scene.add(moon.mesh);
	game.scene.add(earth.mesh);
	game.camera.position.set(0,100,300);
	game.camera.lookAt(0,0,0);
	let center = new THREE.Vector3(0,0,0);
	let sun_light = new THREE.PointLight(0xdd90a5, 5, 0);
	sun_light.position.set(20, 0, 0);
	game.scene.add(sun_light);
	let pair_sun_light = new THREE.PointLight(0xff0000, 5, 0);
	pair_sun_light.position.set(-20, 0, 0);
	game.scene.add(pair_sun_light);
	...
	function animate() {
		...
		pair_sun.rotation.y += 0.01;
		moon.get_rotation().y += 0.03;
		earth.mesh.applyQuaternion(rotation);
		rotateAboutPoint(sun.mesh, center, revAxis, sun.revolution);
		rotateAboutPoint(sun_light, center, revAxis, sun.revolution);
		rotateAboutPoint(pair_sun, center, revAxis, sun.revolution);
		rotateAboutPoint(pair_sun_light, center, revAxis, sun.revolution);
